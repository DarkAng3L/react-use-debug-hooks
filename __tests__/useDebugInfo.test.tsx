  ...
  return {
    ...
    checkConsoleLog: (componentName: string, ...args: any[]) => {
      expect(consoleLogSpy).toHaveBeenNthCalledWith(
        ...
        ...(!!args && args.length > 0 ? [
          `%c[debug-info] %c<${componentName} /> ${ args[0].renderCount ? 'Re-rendered' : (isEmpty(args[0]) ? 'Unmounted' : 'Mounted')}`,
          "color: orange",
          expect.any(String),
          ...(isEmpty(args[0]) ? [] : args),
        ] : [ componentName ])
...
function TestOnlyChangedPropsComponent() {
  ...
  useDebugInfo('TestOnlyChangedPropsComponent', {
    string,
    number,
  }, true);
...
describe('useDebugInfo', () => {
  ...
  test('outputs only the logs when props change', async () => {
    const { user, checkConsoleLog } = renderWithSetup(<TestOnlyChangedPropsComponent />);
